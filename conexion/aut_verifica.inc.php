...
if (isset($_POST['user']) && isset($_POST['pass']) && isset($_POST['code'])) {
 ...
 // chequeamos que se introdujo el codigo aleatorio.
    ...
    //agregada por mi
    $usuario_consulta=pg_query($con, "SELECT * FROM $sql_tabla WHERE usuario='".$_POST['user']."'") or die(pg_last_error());
    ...
    $usuario_datos=pg_fetch_array($usuario_consulta);
        ...
        $_SESSION['id']=$usuario_datos['cedula_usuario'];
        ...
        $_SESSION['username']=$usuario_datos['nombre_usuario']." ".$usuario_datos['apellido_usuario'];
        
        //definimos codigo_aleatorio con el codigo de acceso del usuario.
        $_SESSION['id_codigo']=$cod_aleatorio;
        ...
        //definimos fecha de ultimo acceso del usuario.
        $_SESSION['solicitar_punto']=$usuario_datos['solicitar_punto'];

        //definimos fecha de ultimo acceso del usuario.
        $_SESSION['recibir_punto']=$usuario_datos['recibir_punto'];
        
        $ced_usuario=$usuario_datos['cedula_usuario'];
        $fecha_acceso=date("Y-m-d H:i:s");
        $update_usuario=pg_query($con, "UPDATE $sql_tabla SET fecha_ultimoacceso='NOW', status_login='on' WHERE cedula_usuario='$ced_usuario'") or die(pg_last_error());    
        pg_close($con);
    ...
    if (!isset($_SESSION['usuario_id']) && !isset($_SESSION['usuario_login']) && !isset($_SESSION['usuario_password']) && !isset($_SESSION['id_codigo'])){
        // Borramos la sesion creada por el inicio de session anterior
        $_SESSION = array();
        session_destroy();
        header ("Location: index.php?view=login&msg_login=6");
