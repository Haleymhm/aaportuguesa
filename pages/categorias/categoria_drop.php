...
<?php 
    if (isset($_GET['cod_categoria'])){
        $datos_borrar= $_GET['cod_categoria'];
        
        $query="SELECT * FROM categorias WHERE categorias.cod_categoria ='$datos_borrar' AND categorias.cod_categoria IN (SELECT cod_categoria FROM tramites)";			
        ...
        if ($total_registros == 1){
            ...
            $result_borrar=pg_query("SELECT drop_categoria($datos_borrar)") or die(pg_last_error());
            pg_close();
        }
