...
if (isset($_GET['rif_empresa'])){
	$datos_borrar= $_GET['rif_empresa'];

	//se le hace el llamado al archivo de conexion y luego se realiza el enlace.	
	require("conexion/aut_config.inc.php");
	$db_conexion=pg_connect("host=$sql_host dbname=$sql_db user=$sql_usuario password=$sql_pass");	

	$query = "SELECT * FROM empresa WHERE empresa.rif_empresa ='$datos_borrar' AND empresa.rif_empresa IN (SELECT rif_empresa FROM cuenta) OR empresa.rif_empresa IN (SELECT rif_empresa FROM concepto_factura) ";			
	...
	if ($total_registros == 1){
		$error="malo";	
            ...
            $result_borrar=pg_query("SELECT drop_empresa('$datos_borrar')") or die(pg_last_error());
            pg_close();
	}
}
?> 
