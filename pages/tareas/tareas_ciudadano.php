...
//        echo "<script type='text/javascript'>window.location.href='index.php'</script>";
    ...
    $query="SELECT COUNT(*) FROM ticket, detalles_ticket WHERE ticket.cod_ticket=detalles_ticket.cod_ticket and ticket.prioridad_ticket = 3 and ticket.cedula_rif='$_SESSION['id']'";
    $result = pg_query($query)or die(pg_last_error());  
    $resultados_ticket_urgentes=pg_fetch_row($result);  
    pg_free_result($result);

    //Informacion de Tickets Alta
    $query="SELECT COUNT(*) FROM ticket, detalles_ticket WHERE ticket.cod_ticket=detalles_ticket.cod_ticket and ticket.prioridad_ticket = 2 and ticket.cedula_rif='$_SESSION['id']'";
    $result = pg_query($query)or die(pg_last_error());  
    $resultados_ticket_alta=pg_fetch_row($result);  
    pg_free_result($result);

    //Informacion de Tickets Normal
    $query="SELECT COUNT(*) FROM ticket, detalles_ticket WHERE ticket.cod_ticket=detalles_ticket.cod_ticket and ticket.prioridad_ticket = 1 and ticket.cedula_rif='$_SESSION['id']'";
    $result = pg_query($query)or die(pg_last_error());  
