...
//        echo "<script type='text/javascript'>window.location.href='index.php'</script>";
    ...
    $query="SELECT COUNT(*) FROM ticket WHERE prioridad_ticket = 3";               
    $result = pg_query($query)or die(pg_last_error());  
    $resultados_ticket_urgentes=pg_fetch_row($result);  
    pg_free_result($result);

    //Informacion de Tickets Alta
    $query="SELECT COUNT(*) FROM ticket WHERE prioridad_ticket = 2";               
    $result = pg_query($query)or die(pg_last_error());  
    $resultados_ticket_alta=pg_fetch_row($result);  
    pg_free_result($result);

    //Informacion de Tickets Normal
    $query="SELECT COUNT(*) FROM ticket WHERE prioridad_ticket = 1";               
    $result = pg_query($query)or die(pg_last_error());  
