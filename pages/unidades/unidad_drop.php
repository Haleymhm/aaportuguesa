...
//        echo "<script type='text/javascript'>window.location.href='index.php'</script>";
	...
	$datos_borrar= $_GET['cod_unidad'];
        
        $query="SELECT * FROM unidades WHERE unidades.cod_unidad ='$datos_borrar' AND unidades.cod_unidad IN (SELECT cod_unidad FROM detalles_ticket WHERE detalles_ticket.cod_unidad ='$datos_borrar') OR unidades.cod_unidad IN (SELECT cod_unidad FROM tramites WHERE tramites.cod_unidad ='$datos_borrar')";			
        ...
        if ($total_registros == 1){
            ...
            $result_borrar=pg_query("SELECT drop_unidad($datos_borrar)") or die(pg_last_error());
            pg_close();
        }
    }
?> 
